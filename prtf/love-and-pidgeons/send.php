...
if (strlen($phone) > 10 ) {	
	...
	// Отправка заявки в callTouch START
		 ...
		 curl_setopt($ch, CURLOPT_POST, 1);
		 curl_setopt($ch, CURLOPT_POSTFIELDS,
		 	"phoneNumber=$phone&personalPhone=false&orderComment=".$_SERVER['HTTP_USER_AGENT'].", ".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."".($call_value != 'undefined' ? "&sessionId=".$call_value : "").($testing == true ? "&subject=TESTING" : ""));
