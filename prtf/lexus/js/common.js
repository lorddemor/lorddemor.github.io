...
new Vue({
	...
	computed: {
		...
		lastDayOfMonth: function(m) {
			var y = new Date().getYear();
			var date = new Date(y, this.thisMonth + 1, 0);
			return date.getDate();
		}
	},
	created: function() {
	...
	mounted: function() {
		var w = document.getElementById('available').getElementsByClassName('tile')[0].clientWidth;
		this.tileCircles = parseInt(w / 16);
		...
		if (this.screenSize.x < 767) {
			var g1 = document.getElementById('benefits').getElementsByClassName('benefits')[0];
			var flkty1 = new Flickity(g1, {
				// options
				cellAlign: 'left',
				contain: true,
			...
			var g2 = document.getElementById('available').getElementsByClassName('tabs')[0];
			var flkty2 = new Flickity(g2, {
				// options
				cellAlign: 'left',
				contain: true,
				cellSelector: '.tab',
			...
			var self = this;
			flkty2.on('settle', function() {
				self.changeModel(self.data.models[flkty2.selectedIndex].id, flkty2.selectedIndex);
			});

			var g3 = document.getElementById('benefits-secondary').getElementsByClassName('benefits')[0];
			var flkty3 = new Flickity(g3, {
				// options
				cellAlign: 'left',
				contain: true,
				cellSelector: '.benefit',
...
function googleMap2() {
	if ($(window).width() <= 480) {
		var centrx = '55.721828';
		var centry = '37.699776';
	} else {
		var centrx = '55.721828';
		var centry = '37.699776';
	}
	...
	function e() {
		var e = {
				zoom: 13,
				center: new google.maps.LatLng(centrx, centry),
				styles: [{
		...
		new google.maps.Marker({
			position: new google.maps.LatLng(55.721828, 37.699776),
			map: o,
			title: 'Москва,Остаповский проезд, 4',
			icon: "img/pin.png"
		})
	}
