...
$token = "1539403205ctda4e85fe5d051de797ab1e757052bd49";
$node = "node3";
$ct_id = "18706";
...
curl_setopt($ch, CURLOPT_URL,"http://fastcpa.savemytime.ru/t_bot/get_order_data.php");
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS,
"url=" . $_SERVER['SERVER_NAME'] . "&ct_token=" . $token . "&ct_node=" . $node . "&ct_id=" . $ct_id . "&phone=$phone");
...
if (strlen($phone) > 10 ) {	
	...
	// Отправка заявки в callTouch START
		...
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS,
			"clientApiId=$token&phoneNumber=$phone&personalPhone=false&orderComment=".$_SERVER['HTTP_USER_AGENT'].", ".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."".($call_value != 'undefined' ? "&sessionId=".$call_value : "").($testing == true ? "&orderComment=Тестирование" : ""));
