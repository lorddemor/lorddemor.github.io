...
(function(factory) {
   ...
   ********************/
        ...
        _generateMarkup: function() {
            ...
            if(this.telInput.hasClass('revPhone')) {
                this.PlaceholderContainer = $("<label>", {
                    "for": "rephone"
                }).insertAfter(this.telInput);
                this.PlaceholderContainer.html('Введите номер телефона для обратной связи');
            }
        ...
        _initMask: function(value) {
            this.telInput = $(this.element);
            this.siblingsFlag = this.telInput.siblings('.flag-dropdown').children('.selected-flag').data('code');
                if (!this.telInput.val().length > 0) {
                    this.telInput.val(this.siblingsFlag);
                }

            var maskList = $.masksSort(maskListnF, ['#'], /[0-9]|#/, "mask");
            var maskOpts = {
                inputmask: {
                    definitions: {
                        '#': {
                ...
                onMaskChange: function(maskObj, completed) {
                    if (completed) {
                        ...
                        var bmask = mask.replace(/#/g,'_');
                        $(this).data('mask',bmask);
            ...
            this.telInput.inputmasks(maskOpts);
        },
        ...
        _appendListItems: function(countries, className) {
            ...
            for (var i = 0; i < countries.length; i++) {
                ...
                if (this.isMobile) {
                    ...
                    // add the flag
                    tmp += "<div class='flag'><div style='background-image: url(/local/templates/GetBuketMobile/pic/flags.png);' class='iti-flag lazyload " + c.iso2 + "'></div></div>";
        ...
        _initKeyListeners: function() {
            ...
            if (this.options.autoFormat) {
                ...
                this.telInput.on("keyup" + this.ns, function(e) {
                    if(e.which == keys.BSPACE && $(this).val() == "+7(___)___-__-__") {
                        e.preventDefault();
                        return false;
                    } else {
                        // change($(this).val());
                        ...
                        if (e.which >= keys.SPACE && !e.ctrlKey && !e.metaKey && window.intlTelInputUtils && !that.telInput.prop("readonly")) {
                            ...
                            var isAllowedKey = e.which >= keys.ZERO && e.which <= keys.NINE || e.which == keys.PLUS,
                                ...
                                max = that.telInput.attr("maxlength"), val = that.telInput.val(), // assumes that if max exists, it is >0
        ...
        _initFocusListeners: function() {
            ...
            this.telInput.on("click" + this.ns, function(e) {
                var str =  that.telInput.val();
                ...
                if(result) {
                    ...
                    that.telInput.selectRange(result.index, result.index);
                }
            });
        ...
        _selectFlag: function(countryCode, updateDefault) {
            ...
            this.selectedFlagInner.attr("class", "iti-flag lazyload " + countryCode);
            this.selectedFlagInner.attr("style", "background-image: url('/local/templates/GetBuketMobile/pic/flags.png');");
            // update the selected country's title attribute
            var title = countryCode ? this.selectedCountryData.name + ": +" + this.selectedCountryData.dialCode : "Unknown";
            this.selectedFlagInner.parent().attr("title", title);
            this.selectedFlagInner.parent().attr('data-code', this.selectedCountryData.dialCode);
        ...
        _selectListItem: function(listItem) {
            ...
            if(result) {
                ...
                this.telInput.selectRange(result.index, result.index);
            }
        },
